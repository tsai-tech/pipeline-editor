...
эмитит снапшоты по мере переходов узлов idle → running → success/error/skipped.
Моделирует:

- **реальные выражения** — триггеры фейковые (сразу пушат форму из каталога), но
  мок **вычисляет** язык выражений (`expression.ts`, без `eval`): `$json`/
  `$node["Title"]`, доступ по путям, операторы, шаблоны `{{ }}`. Так control-flow
  (if/switch/filter) реально маршрутизирует, Set Fields вытаскивает поле из
  контекста, effect резолвит свои параметры. Доступ в изменившуюся форму (путь
  пропал) **кидает** → нода падает (как в n8n) — фатально, кроме необязательного
  effect; ошибка подсвечивается на ноде и в инспекторе. Это тестирует и сам язык;
- **один сработавший триггер за прогон** — у пайплайна может быть несколько
  триггеров, но событие приходит из одного канала: мок выбирает один (round-robin
  по кликам Run или явно), остальные — `skipped`, поэтому маршрут «по триггеру»
  осмыслен;
- **control-flow** — исполняется только «взятая» ветка; узлы за не взятой ветвью
